...
export const AdminPanel: React.FC<AdminPanelProps> = ({ tasks, users, channels, onCreateTask, onEditTask, onDeleteTask, viewingMember, onBackToDashboard }) => {
    ...
    const MainDashboard = () => {
        ...
        const StatCard = ({ title, value, color, icon }: any) => (
            <div className="bg-[#151725]/60 backdrop-blur-xl border border-white/5 p-6 rounded-[1.5rem] relative overflow-hidden group hover:border-white/10 transition-all duration-300 hover:shadow-[0_0_30px_rgba(0,0,0,0.3)]">
                <div className={`absolute top-0 right-0 w-32 h-32 bg-${color}-500/20 rounded-full blur-3xl -mr-10 -mt-10 group-hover:bg-${color}-500/30 transition-colors`}></div>
                <div className="relative z-10">
                    <div className="flex items-center justify-between mb-4">
                         <div className={`p-3 rounded-2xl bg-${color}-500/10 text-${color}-400 border border-${color}-500/20 shadow-lg`}>
                            {icon}
                        </div>
                        <p className={`text-${color}-300 text-[10px] font-bold uppercase tracking-widest`}>{title}</p>
                    </div>
        ...
        const TaskList = ({ title, tasklist }: { title: string, tasklist: Task[]}) => (
            <div className="bg-[#151725]/60 backdrop-blur-xl border border-white/5 rounded-[1.5rem] overflow-hidden flex flex-col h-full min-h-[300px] shadow-lg">
                <div className="px-6 py-5 border-b border-white/5 bg-white/[0.02] flex justify-between items-center">
                    ...
                    <span className="text-xs bg-white/10 px-2.5 py-1 rounded-full text-slate-300 font-mono border border-white/5">{tasklist.length}</span>
                </div>
                <div className="divide-y divide-white/5 overflow-y-auto max-h-[400px] lg:max-h-none custom-scrollbar flex-1">
                    {tasklist.length > 0 ? tasklist.map(task => (
                        <div key={task.id} className="p-4 flex items-center justify-between hover:bg-white/[0.02] transition-colors group">
                            ...
                            <div className="flex items-center space-x-1 lg:opacity-0 lg:group-hover:opacity-100 transition-opacity lg:translate-x-2 lg:group-hover:translate-x-0">
        ...
        const UserManagementList = () => (
             <div className="bg-[#151725]/60 backdrop-blur-xl border border-white/5 rounded-[1.5rem] overflow-hidden flex flex-col h-full min-h-[300px] shadow-lg">
                <div className="px-6 py-5 border-b border-white/5 bg-white/[0.02] flex justify-between items-center">
                    ...
                    <span className="text-xs bg-white/10 px-2.5 py-1 rounded-full text-slate-300 font-mono border border-white/5">{users.length}</span>
                </div>
                <div className="divide-y divide-white/5 overflow-y-auto max-h-[400px] lg:max-h-none custom-scrollbar flex-1">
                    {users.map(user => (
                        <div key={user.id} className="p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between hover:bg-white/[0.02] transition-colors gap-3">
                             <div className="flex items-center space-x-3">
                                <div className={`h-8 w-8 rounded-lg flex items-center justify-center text-xs font-bold ${user.role === Role.PATRAO ? 'bg-cyan-500 text-white shadow-lg shadow-cyan-500/20' : 'bg-slate-700 text-slate-300 border border-white/5'}`}>
                                    {user.name.charAt(0)}
                                </div>
                                <div>
                            ...
                            <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end pl-11 sm:pl-0">
                                ...
                                <button 
                                    ...
                                    className="text-[10px] font-bold text-slate-400 hover:text-white border border-white/10 hover:border-white/30 px-3 py-1.5 rounded-lg transition-all"
                                >
                                    Trocar Cargo
                                </button>
                            </div>
        ...
        return (
            <div className="max-w-7xl mx-auto space-y-6 md:space-y-8 p-4 md:p-6 overflow-y-auto">
                <header className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-2xl md:text-3xl font-bold text-white tracking-tight">Visão Geral</h1>
                        <p className="text-slate-400 text-sm mt-1">Gerencie o desempenho e as tarefas da equipe.</p>
                    </div>
                    <button onClick={handleOpenCreateModal} className="w-full md:w-auto bg-gradient-to-r from-blue-600 to-cyan-500 hover:from-blue-500 hover:to-cyan-400 text-white font-semibold py-3 px-6 rounded-xl transition-all shadow-[0_0_20px_rgba(42,106,178,0.4)] hover:shadow-[0_0_30px_rgba(91,197,242,0.6)] active:scale-95 flex items-center justify-center space-x-2 border border-white/10">
                ...
                {/* On mobile, use auto height so it scrolls with the page. On desktop, use fixed height for layout consistency */}
                <section className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8 h-auto lg:h-[500px]">
    ...
    const MemberDetailPanel = ({ member }: { member: User }) => {
        ...
        const ReadOnlyTaskList = ({ title, tasklist }: { title: string, tasklist: Task[]}) => (
             <div className="bg-[#151725]/60 backdrop-blur-xl border border-white/5 rounded-[1.5rem] overflow-hidden flex flex-col h-full min-h-[300px]">
            ...
            <div className="max-w-5xl mx-auto p-4 md:p-6 overflow-y-auto">
                <header className="flex flex-col md:flex-row items-start md:items-center justify-between mb-8 md:mb-10 gap-4">
                    <div className="flex items-center">
                        ...
                        <div>
                             <h1 className="text-2xl md:text-3xl font-bold text-white">Perfil do Colaborador</h1>
                             <p className="text-slate-400 text-xs md:text-sm mt-1">Análise detalhada de <span className="text-cyan-400 font-semibold">{member.name}</span></p>
                        </div>
                    </div>
                    {member.role === Role.MEMBRO ? (
                        <button 
                            onClick={handlePromote}
                            className="w-full md:w-auto bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 text-white font-semibold py-2 px-6 rounded-xl shadow-lg shadow-purple-900/40 transition-all text-sm flex items-center justify-center gap-2"
                        >
                         ...
                         <button 
                            ...
                            className="w-full md:w-auto bg-slate-700 hover:bg-slate-600 text-slate-300 font-semibold py-2 px-6 rounded-xl transition-all text-sm flex items-center justify-center gap-2 border border-slate-600"
                        >
                 ...
                 <section className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 mb-8 md:mb-10">
                    <div className="bg-[#151725]/60 backdrop-blur-xl p-6 md:p-8 rounded-[2rem] border border-white/5 text-center shadow-lg relative overflow-hidden">
                        ...
                        <p className="text-4xl md:text-5xl font-bold text-white tracking-tighter">{member.points}</p>
                    </div>
                    <div className="bg-[#151725]/60 backdrop-blur-xl p-6 md:p-8 rounded-[2rem] border border-white/5 text-center shadow-lg relative overflow-hidden">
                        ...
                        <p className="text-4xl md:text-5xl font-bold text-white tracking-tighter">{memberTasks.length}</p>
                    </div>
                    <div className="bg-[#151725]/60 backdrop-blur-xl p-6 md:p-8 rounded-[2rem] border border-white/5 text-center shadow-lg relative overflow-hidden">
                        ...
                        <p className="text-4xl md:text-5xl font-bold text-white tracking-tighter">{completionRate}%</p>
                    </div>
                </section>

                <section className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 h-auto lg:h-[400px]">
